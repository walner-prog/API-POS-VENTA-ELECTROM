...
/*
export async function crearVentaService({ carrito, cliente_nombre, observacion, paga_con }, usuario_id) {
    const t = await sequelize.transaction();
    const start = Date.now();
    ...
    try {
        ...
        // 🔹 Calcular cambio
        const pagoCliente = parseFloat(paga_con) || 0;
        const cambio = Math.max(0, (pagoCliente - totalVenta)).toFixed(2);
        ...
        const venta = await Venta.create({
            ...
            iva: totalIva,
            total: totalVenta,
            paga_con: pagoCliente,
            cambio,  // ✅ se guarda el cambio
            observacion,
            caja_id: cajaAbierta.id,
            usuario_id
        }, { transaction: t });
